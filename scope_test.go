...
func TestScope_DeadlineTaskCompletesInTime(t *testing.T) {
	ctx := context.Background()

	err := Run(ctx, func(g *Grove) error {
		// create a scope of 100 ms timeout
		scope := g.WithDeadline(time.Now().Add(50 * time.Millisecond))
		scope.Go("task-20ms", func(ctx context.Context) error {
...
func TestScope_DeadlineTaskExceedsTimeout(t *testing.T) {
	ctx := context.Background()
...
func TestScope_DeadlineDoesNotCancelSiblings(t *testing.T) {
	ctx := context.Background()
...
func TestScope_TwoTasksWithDifferentDeadlines(t *testing.T) {
	var me MultiError
	ctx := context.Background()
	...
	err := Run(ctx, func(g *Grove) error {
		// scope 1 (50 ms)
		scope1 := g.WithDeadline(time.Now().Add(50 * time.Millisecond))
		scope1.Go("long-task-scope1", func(ctx context.Context) error {
			<-ctx.Done()
			return ctx.Err()
		})
		...
		// scope 2 (30 ms)
		scope2 := g.WithDeadline(time.Now().Add(50 * time.Millisecond))
		scope2.Go("long-task-scope2", func(ctx context.Context) error {
			<-ctx.Done()
			return ctx.Err()
		})
...
func TestScope_ParentCancelledBeforeScopeDeadline(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
